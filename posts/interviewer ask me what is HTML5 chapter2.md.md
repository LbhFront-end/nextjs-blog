...
| 参数 | 描述 |
| :------- '| :------------------------------ |'
| _x_ | 开始复制的左上角位置的 x 坐标。 |
| _y_ | 开始复制的左上角位置的 y 坐标。 |
| _width_ | 将要复制的矩形区域的宽度。 |
| _height_ | 将要复制的矩形区域的高度。 |
...
| 参数 | 描述 |
| :------------ '| :---------------------------------------------------- |'
| _imgData_ | 规定要放回画布的 ImageData 对象。 |
| _x_ | ImageData 对象左上角的 x 坐标，以像素计。 |
| _y_ | ImageData 对象左上角的 y 坐标，以像素计。 |
| _dirtyX_ | 可选。水平值（x），以像素计，在画布上放置图像的位置。 |
| _dirtyY_ | 可选。水平值（y），以像素计，在画布上放置图像的位置。 |
| _dirtyWidth_ | 可选。在画布上绘制图像所使用的宽度。 |
| _dirtyHeight_ | 可选。在画布上绘制图像所使用的高度。 |
...
```html
<！DOCTYPE html>
<html mainfest="index.appcache"></html>
```
