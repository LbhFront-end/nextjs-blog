...
Lazy-Load 就是懒加载。是针对图片加载时间的优化，适用于图片量比较大的网站。用户打开页面的时候如果把所有图片资源加载完毕，可能会造成白屏、卡顿等现象，浏览器没有办法一次性处理这么多任务。实际上用户打开页面的瞬间也不需要所有图片，呈现给用户的只有首屏，下面的图片可以等用户下拉的瞬间再计时去请求。
...
style 内联样式中，背景图片设置了 none,这就是说这个 div 是没有内容的，只是用了占位作用。一旦通过滚动使得这个 div 出现在了可见范围内，div 元素的内容就会发生变化：

```html
<div style="background-image: url(&quot;https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/9/27/16619f449ee24252~tplv-t2oaga2asx-image.image;); background-size: cover;" />
```

内联样式中的背景图片属性从 none 变成了一个在线图片的 URL。这就是懒加载的实现思路
...
懒加载实现中，有两个关键的数值：一个是当前可视区域的高度，另一个是元素距离可视区域顶部的高度。

当前可视区域的高度，可以用 window.innerHeight 属性获取，兼容旧版本用 document.documentElement.clientHeight获取

```javascript
const viewHeight = window.innerHeight || document.documentElement.clientHeight
```

元素距离可视区域顶部的高度，用 getBoundingClientRect 方法来获取元素的大小及其相对视口的位置。

> 该方法的返回值是一个 DOMRect 对象，这个对象是由该元素的 getClientRects 方法返回的一组矩形的集合，是与该元素相关的 CSS 边框集合
>
> DOMRect 对象包含了一组用于描述边框的只读属性——left/top/right/bottom 单位为像素，除了 width/height 外的属性都是相对于视口的左上角位置而言的
...
## 应用篇2：事件的节流(throttle)与防抖(debounce)

懒加载效果中 scroll 事件是一个非常容易被反复触发的事件，但其实不止 scroll事件，还有 resize 事件、鼠标事件、键盘事件等等。

频繁触发回调导致的大量计算会引起页面的抖动甚至卡顿，为了规避这种情况，我们需要手段来控制事件被触发的频率。

### 节流与防抖的本质

都以闭包的形式存在，通过对事件对应的回调函数进行包裹、以自由变量的形式缓存时间信息，最后用 setTimeout 来控制事件的触发频率

### Throttle：第一个人说了算

在某段时间不管触发多少次回调，都只认第一次，并在结束时给予响应。

```javascript
// fn 是包裹的事件回调，interval 是时间间隔的阈值
function throttle(fn, interval) {
    ...
    return function () {
        ...
        // 记录本次触发回调的时间
        let now = +new Date();
        // 判断上次触发的时间和本次的触发时间差是不是小于时间间隔的阈值
        if (now - last >= interval) {
            // 如果时间间隔大于设定的时间阈值，则执行回调
            last = now;
            fn.apply(context, args)
        }
    }
}
const betterScroll = throttle(() => console.log('触发了滚动事件'), 1000);
document.addEventListener('scroll', betterScroll);
```

### Debounce: 最后一个人说了算

某段时间内，不管触发了多次回调，都只认最后一次。
...
const betterScroll = debounce(() => console.log('触发了滚动事件'), 1000);
document.addEventListener('scroll', betterScroll);
```

### 用 Throttle 来优化 Debounce

debounce 的问题在于太过耐心，如果用户操作十分频繁，不等 delay 时间结束就继续下一次操作，那么定时器不断刷新，频繁的延迟导致用户迟迟得不到响应，用户同样会产生这个页面卡死的错觉。

为了避免弄巧成拙，借用 throttle 的思想，在 delay 时间内重新生成定时器，但只要 delay 的时间到了就给用户一个响应。

```javascript
// fn 是包裹的事件回调，interval 是时间间隔的阈值
function throttle(fn, delay) {
    // last 为上一次触发回调的时间 timer 为定时器
    ...
    return function () {
        ...
        // 判断上次触发的时间和本次的触发时间差是不是小于时间间隔的阈值
        if (now - last < delay) {
            // 如果时间间隔小于设定的时间阈值，则为本次触发操作设定一个定时器
            clearTimeout(timer)
            timer = setTimeout(function () {
                last = now;
                fn.apply(context, args)
            ...
            // 如果时间间隔超出设定的时间阈值，那就不等了，直接给用户一次响应
            last = now;
            fn.apply(context, args)
        }
    }
