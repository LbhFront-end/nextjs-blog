...
01. 如果是布尔值，`true` 和 `false`分别被转换为 1 和 0
02. 如果是数字值，返回本身
03. 如果是 `null`,返回 0.  如果是 `undefined`，返回 `NaN`
04. 如果是字符串，遵循以下规则：
...
05. 如果是对象，则调用对象的 `valueOf()` 方法，然后按照前面的规则进行转换返回的值，如果是转换结果是 `NaN`, 则调用对象的 `toString()` 方法，然后再一次按照前面的规则进行返回的字符串值的转换
...
01. 忽略前面字符串前面的空格，直至找到第一个非空格字符
02. 如果第一个字符不是数字字符或者负号，就会返回 `NaN`（也就是遇到空字符会返回 NaN）
03. 如果第一个字符是数字字符，会继续解析第二个字符，知道解析完所有后续的字符或者是遇到一个非数字字符
04. 如果字符串中第一个字符是数字字符，也能够识别各种进制
05. 最好在 第二个参数指定转换的基数（进制），就不会有所歧义。
...
01. 如果有toString()方法，则调用该方法（不传递radix参数）并返回结果
02. 如果是null，返回”null”
03. 如果是undefined，返回”undefined”
...
01. 如果是包含有效数字字符的字符串，先将其转换为数字值（转换规则同 `Number()`），再执行加减1的操作，字符串变量变为数值变量。
02. 如果是不包含有效数字字符的字符串，将变量的值设置为 `NaN`，字符串变量变成数值变量。
03. 如果是布尔值 `false`，先将其转换为0再执行加减1的操作，布尔值变量编程数值变量。
04. 如果是布尔值 `true`，先将其转换为1再执行加减1的操作，布尔值变量变成数值变量。
05. 如果是浮点数值，执行加减1的操作。
06. 如果是对象，先调用对象的 `valueOf()` 方法，然后对该返回值应用前面的规则。如果结果是 `NaN`，则调用 `toString()` 方法后再应用前面的规则。对象变量变成数值变量。
...
01. 如果一个操作数为 `NaN`，则结果为 `NaN`
02. 如果是 `Infinity+Infinity`，结果是 `Infinity`
03. 如果是 `-Infinity+(-Infinity)`，结果是 `-Infinity`
04. 如果是 `Infinity+(-Infinity)`，结果是 `NaN`.
05. 如果是 `+0+(+0)`，结果为 `+0`
06. 如果是 `(-0)+(-0)`，结果为 `-0`.  
07. 如果是 `(+0)+(-0)`，结果为 `+0`
...
01. 如果两个操作值都是字符串，则将它们拼接起来 如果只有一个操作值为字符串，则将另外操作值转换为字符串，然后拼接起来2.  如果一个操作数是对象、数值或者布尔值，则调用toString()方法取得字符串值，然后再应用前面的字符串规则。
02. 对于undefined和null，分别调用String()显式转换为字符串。
...
01. 如果第一个操作值经 `Boolean()` 转换后为 `true`，则返回第二个操作值，否则返回第一个值（不是`Boolean()` 转换后的值）
02. 如果第一个操作值是对象，则返回第二个操作值
03. 如果两个操作值都是对象，则返回第二个操作值
04. 如果有一个操作值为 `null`，返回 `null`
05. 如果有一个操作值为 `NaN`，返回 `NaN`
06. 如果有一个操作值为 `undefined`，返回 `undefined`

逻辑或（||）操作符，如果一个操作值不是布尔值，遵循以下规则

01. 如果第一个操作值经 `Boolean()` 转换后为 `false`，则返回第二个操作值，否则返回第一个操作值（不是`Boolean()` 转换后的值）
02. 如果两个操作值都是对象，则返回第一个操作值
03. 如果两个操作值都为 `null`，返回 `null`
04. 如果两个操作值都为 `NaN`，返回 `NaN`
05. 如果两个操作值都为 `undefined`，返回 `undefined`

##### 关系操作符（&lt; , &gt; , &lt; =, &gt; =）

与上述操作符一样，关系操作符的操作值也可以是任意类型的，所以使用非数值类型参与比较时也需要系统进行隐式类型转换：

01. 如果两个操作值都是数值，则进行数值比较
02. 如果两个操作值都是字符串，则比较字符串对应的字符编码值
03. 如果只有一个操作值是数值，则将另一个操作值转换为数值，进行数值比较
04. 如果一个操作数是对象，则调用 `valueOf()` 方法（如果对象没有 `valueOf()` 方法则调用`toString()` 方法），得到的结果按照前面的规则执行比较
05. 如果一个操作值是布尔值，则将其转换为数值，再进行比较
...
01. 如果一个操作值为布尔值，则在比较之前先将其转换为数值
02. 如果一个操作值为字符串，另一个操作值为数值，则通过Number()函数将字符串转换为数值
03. 如果一个操作值是对象，另一个不是，则调用对象的 `valueOf()` 方法，得到的结果按照前面的规则进行比较
04. null` 与 `undefined` 是相等的`

05. 如果一个操作值为 `NaN`，则相等比较返回 `false`
06. 如果两个操作值都是对象，则比较它们是不是指向同一个对象
...
01.  可以通过角标调用，如 `array[0]`2.  具有长度属性`length`3.  可以通过 for 循环和`forEach`方法进行遍历类数组顾名思义，具备的特征应该与数组基本相同，那么可以知道，一个形如下面这个对象的对象就是一个类数组
...
01. 函数的局部集合，只是这些局部变量在函数返回后会继续存在
02. 函数的“堆栈”在函数返回后并不释放，可以理解为这些函数堆栈并不在栈上分配而是在堆上分配
03. 当在一个函数内部定义另外一个函数就会产生闭包
...
01.  **在冒泡事件和捕获事件同时存在的情况下，捕获事件优先级高一点**
02.  **在同一个元素的绑定事件中，冒泡和捕获没有次序之分，遵循Javascript的执行顺序。**
03.  **在元素上同时绑定捕获事件和冒泡事件，如果通过此元素的子级元素触发，则优先触发捕获事件，若不通过此元素的子级元素触发，则按照Javascript执行顺序触发。**
...
01. 如果两个值类型相同，再进行三个等号（===）的比较
02. 如果两个值类型不同，也有可能相等，需根据以下规则进行类型转换再比较

      01. 如果一个是 null,一个是 undefined 那么相等

      02. 如果一个是字符串，一个是数值，把字符串转换成数值之后再进行比较

三等号 === ：

01. 如果类型不同，就一定不相等
02. 如果两个都是数值，并且是同一个值，那么相等；如果其中至少一个是 NaN,那么不相等（判断一个值是否是 NaN,只能使用 isNaN() 来判断）
03. 如果两个都是字符串，每个位置的字符都一样，那么相等，否则不相等
04. 如果两个值都是 true,或是 false,那么相等
05. 如果两个值都引用同一个对象或者是函数，那么相等，否则不相等
06. 如果两个值都是 null，或者是 undefined 那么相等。
...
01. cookie、LocalStorage 和 IndexDB无法读取
02. DOM 无法获取
03. AJAX请求无法发送
...
01. JS 跨域请求资源会被限制。但是在页面中，script 标签跨域时，却是没有限制的（frame，img同理）。
02. 我们通过，script的src属性，请求服务器，并通过参数（如：？callback=foo，foo为本地一个执行的方法）告诉服务器返回指定格式的JS脚本，并将数据封装在此脚本中。
03. 服务器再配合客户端返回一段脚本 （如：_ foo({“id”: 123, “name” : 张三, “age”: 17});_ ），其实返回的就是 一个客户端本地的一个可执行的方法的方法名, 并将要返回的 数据封装在了参数里
04. 请求到资源后，本地就会执行此方法，通过对参数的处理，也就获取到了我们所要的数据。
...
01. `innerText`是标签内的文本，输出的是字符串，它可以获得某个节点下面的所有标签内的文本
02. `innerHtml` 可以获得某个DOM 节点内部的 HTML 代码
03. `value`是表单元素的属性，获得是表单元素里面的值
...
01. 这个属性只有函数执行时才有效
02. 它有一个 `length` 属性，可以用来获得形参的个数，因此可以用来比较形参和实参个数是否一致，即比较`arguments.length`是否等于 `argument.callee.length`
03. 它可以用来传递匿名函数
...
01. [JS类型转换(强制和自动的规则)](https://www.cnblogs.com/Juphy/p/7085197.html)
02. [JavaScript 数据类型转换（显式与隐式）](https://www.cnblogs.com/tongtian17/p/6265414.html)
03. [DOM标准与IE的html元素事件模型区别](http://www.cnblogs.com/ilexcai/archive/2011/09/05/2168094.html)
04. [ie和dom事件流的区别](https://www.cnblogs.com/luckyXcc/p/5862759.html)
05. [前端面试题——call与apply方法的异同](https://www.jianshu.com/p/131ce0390cf8)
06. [JavaScript中B继承A的方法](https://www.cnblogs.com/fwei/p/6475973.html)[Me丶微笑](https://www.cnblogs.com/fwei/)
07. [Javascript 中 作用域、闭包与 this指针](https://blog.csdn.net/junbo_song/article/details/52261247)
08. [理解JAVASCRIPT的闭包](https://coolshell.cn/articles/6731.html)
09. [深入理解JS中的变量作用域](https://blog.csdn.net/bbs375/article/details/52483862)
