    ...
    <article className="post post-type-normal">
      <animated.header className="post-header" style={articleHeaderStyle}>
        <h3 className="post-title">
          <Link className="post-title-link" href={`/blog/${post.slug}`}>
      ...
      <div id="posts" className="posts-collapse">
        {!category && !tag && (
          <>
            ...
            <span className="archive-page-counter">非常好! 目前共计 {count} 篇日志。 继续努力。</span>
          </>
        )}
        {!category &&
          !tag &&
            ...
            .map(year => (
              <>
                <animated.div className="collection-title" style={titleStyle}>
                  <h2 className="archive-year" id={`archive-year-${year}`}>
                    {year}
                  </h2>
                </animated.div>
                {posts[year].map(post => (
