...
export default function Sider({ toggle, siderItems = [] }: SiderProps) {
  ...
  const items = [
    { key: 'posts', link: '/archives', count: allPosts.length, name: '日志' },
    {
      key: 'categories',
      link: '/categories',
      count: [...new Set(allPosts.map(item => item.categories).flat(2))].length,
      name: '分类'
    },
      ...
      <div className="site-overview">
        ...
        <div className="links-of-author motion-element">
          <span className="links-of-author-item">
            {authorLinks.map(author => (
              <Link target="_blank" title={author.title} rel="noreferrer" key={author.title} href={author.link}>
                <i className={`fa fa-fw ${author.icon}`}></i>
                {author.title}
              </Link>
            ))}
          </span>
        ...
        <div className="fluid-vids">
          <iframe
            ...
            width="100%"
            height="100"
            src="//music.163.com/outchain/player?type=0&amp;id=2353471182&amp;auto=0&amp;height=90"
          />
        </div>
          ...
          <ul className="links-of-blogroll-list">
            {friendLinks.map(friend => (
              <li className="links-of-blogroll-item" key={friend.title}>
                <Link target="_blank" title={friend.title} rel="noreferrer" href={friend.link}>
                  {friend.title}
                </Link>
              </li>
            ))}
