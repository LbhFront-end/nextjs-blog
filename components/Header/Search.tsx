...
const searchClient = algoliasearch(algolia.APPLICATION, algolia.SEARCH_ONLY_API_KEY);
              ...
              <div className="ais-hits">
                {data?.hits.map(item => (
                  <div className="ais-hits--item algolia-hit-item" key={item.slug}>
                    <Link className="algolia-hit-item-link" href={`/blog/${item.slug}`} onClick={onOk}>
            ...
            {data.hits && data.hits.length > 0 && (
              <div id="algolia-pagination" className="algolia-pagination">
                <ul className="ais-pagination pagination">
                  ...
                  {data?.nbPages &&
                    Array.from(Array(data?.nbPages).keys()).map(page => (
                      <li
                        onClick={() => setQuery({ ...query, page })}
                        key={page}
                        className={classNames('ais-pagination--item pagination-item ais-pagination--item__page', {
                          'ais-pagination--item__active current': page === data?.page
                        })}
                      >
                        <a className="ais-pagination--link page-number" href="#">
                          {page + 1}
                        </a>
                      </li>
                    ))}
